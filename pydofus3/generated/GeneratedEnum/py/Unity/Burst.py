...
class BurstExecutionEnvironment(IntFlag):
    Default = 0
    NonDeterministic = 0
    Deterministic = 1

# Unity.Burst.BurstString
class BurstString(IntFlag):
    Integer = 0
    Float = 1
...
# Unity.Burst.BurstTargetCpu
class BurstTargetCpu(IntFlag):
    Auto = 0
    X86_SSE2 = 1
    X86_SSE4 = 2
    X64_SSE2 = 3
...
class CompilationPriority(IntFlag):
    EagerCompilationSynchronous = 0
    Asynchronous = 1
    ILPP = 2
    EagerCompilationAsynchronous = 3

# Unity.Burst.DiagnosticId
class DiagnosticId(IntFlag):
...
class GlobalSafetyChecksSettingKind(IntFlag):
    Off = 0
    On = 1
    ForceOn = 2

# Unity.Burst.NativeDumpFlags
class NativeDumpFlags(IntFlag):
    NONE = 0
    IL = 1
    Unused = 2
    IR = 4
